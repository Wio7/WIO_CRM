  ...
  return (
    <section className="flex h-full flex-col rounded-xl border border-border bg-card">
      <header className="border-b border-border px-5 py-4">
        <h2 className="text-sm font-semibold text-foreground">Valor del Embudo</h2>
        <p className="mt-0.5 text-xs text-muted-foreground">
          Tratos abiertos por etapa
        </p>
      </header>
      ...
      <div className="flex flex-1 flex-col p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={GitBranch}
            title="Sin tratos abiertos aún"
            hint="Crea tratos en los Embudos para ver el desglose de etapas aquí."
          />
        ) : (
          <>
              ...
              {data.stages.map((s) => (
                <li key={s.id} className="flex items-center gap-3 text-xs">
                  ...
                  <span className="text-muted-foreground tabular-nums">
                    {s.dealCount} {s.dealCount === 1 ? 'trato' : 'tratos'}
                  </span>
