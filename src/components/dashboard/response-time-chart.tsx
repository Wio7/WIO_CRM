    ...
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between gap-3 border-b border-border px-5 py-4">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">
            Minutos para responder al primer mensaje no respondido de un cliente, por día de la semana
          </p>
        </div>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState
            icon={Clock}
            title="Sin respuestas registradas aún"
            hint="Este gráfico se llenará a medida que respondas a los mensajes de los clientes."
          />
        ) : (
          <BarChart
            data={chartData}
